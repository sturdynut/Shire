# Shire — Product Plan

Shire is a small, Mac-only app that makes it easy to use a laptop or desktop Mac as a dependable personal server.

It keeps a few important background services running, keeps the Mac awake when needed, tells you when something breaks, and makes it obvious whether everything is healthy, from the Mac or from your phone.

The core idea:

> Declare what should be running, and Shire keeps the Mac in that desired state.

Shire is **not** a general-purpose process manager or a `launchd` GUI. It stays opinionated around running a small set of personal services on a Mac.

UI designs (light and dark, Mac and phone): https://claude.ai/artifact/NB48q5ZLntxFK8LPiAkVBC

---

## What it will run first

| Service | What | How Shire treats it |
|---|---|---|
| `bagend-api` | Bag End API (`~/Code/bagend/server`), port 3001 | managed, built with `pnpm build`, run with `node dist/index.js` |
| `bagend-web` | Bag End web app (`~/Code/bagend/web`), port 5174 | managed, built with `pnpm build`, served with `pnpm preview`, shared on the tailnet |
| `postgres` | Homebrew `postgresql@16` | **external**: watched, not managed (`brew services` owns it) |
| `redbook` | Red Book, a desktop app kept open with `--remote-debugging-port=9222` | managed GUI app; if it's already open, Shire watches that copy |

**Bag End on this Mac is a staging copy with sample data.** It uses its own local database (`.env.staging`), never production's. Production runs elsewhere.

---

## Primary use cases

- Keep a staging copy of a web app (Bag End) running and reachable from your phone
- Keep a desktop app (Red Book) open with its debug port, without launching a second copy
- Watch services Shire doesn't own (Postgres via `brew services`)
- Ensure Tailscale is available
- Keep the Mac awake while server mode is enabled
- Restart services when they crash, and name the reason
- Show whether each service is actually healthy
- Tell you when something breaks, on the Mac and on your phone
- Check in, and restart a service, from your phone over Tailscale
- Warn about things that stop the Mac serving unattended

---

## Product principles

### 1. YAML is the source of truth

One human-readable config file, easy to keep in Git. Edit it in VS Code, generate it with scripts, commit it to dotfiles. Shire watches the file and validates every save. No form-based editor in v1.

### 2. Use macOS primitives instead of rebuilding them

`launchd` owns process lifecycle. Shire configures and observes it; it does not replace it.

### 3. Opinionated over exhaustive

Support only what personal server workloads need: command, arguments, working directory, environment, restart behavior, build step, dependencies, health checks, logs, Tailscale sharing.

### 4. Health is different from process state

A live process isn't necessarily a usable app. Shire distinguishes stopped, starting, running, unhealthy, healthy and **crash-looping**.

### 5. Strictly local

No account, no hosted control plane, no third-party services. Everything stays on the Mac and your tailnet. **The one exception is phone alerts**, which travel through Apple's push service because that is the only way to reach an iPhone.

### 6. Explain, don't just report

When something fails, say why in plain words and offer the fix. A small fixed set of known failure patterns covers most real problems.

---

# Architecture

## Components

```text
config.yaml ──► shire (CLI) ──► LaunchAgents ──► services
                     │
                     └──► shire-agent (always running, itself a LaunchAgent)
                            ├─ health checks
                            ├─ keep-awake power assertion
                            ├─ readiness checks
                            ├─ alerts
                            └─ phone status page (via tailscale serve)

Shire.app (menu bar + window) ──► talks to shire-agent, shares the Core library
```

The **CLI runs and exits**. Anything continuous (health checks, keep-awake, alerts, the phone page) lives in **shire-agent**, a LaunchAgent generated by Shire like any other, so it survives the menu bar app quitting.

## Config

```text
~/.config/shire/config.yaml
```

```yaml
serverMode: { keepAwake: true }
presets: { tailscale: { enabled: true } }
remote: { statusPage: tailnet, actions: restart }   # restart is on by default

alerts:
  macos: true
  phone: true          # Apple push: the only thing that leaves your tailnet
  crashLoop: 3 in 5m
  unhealthyFor: 2m

logs: { maxSize: 10MB, keep: 3 }

services:
  postgres:
    external: homebrew.mxcl.postgresql@16   # watched, not managed
    health: { type: tcp, port: 5432 }

  bagend-api:
    command: node
    args: [dist/index.js]
    build: pnpm build
    cwd: ~/Code/bagend/server
    envFile: .env.staging    # local staging database, sample data only
    dependsOn: [postgres]
    health: { type: tcp, port: 3001 }

  bagend-web:
    command: pnpm
    args: [preview, --port, "5174"]
    build: pnpm build
    cwd: ~/Code/bagend/web
    serve: 443               # tailscale serve → https://mac-mini.<tailnet>.ts.net
    dependsOn: [bagend-api]
    health: { type: http, url: http://localhost:5174 }

  redbook:
    command: /Applications/RedBook.app/Contents/MacOS/RedBook
    args: [--remote-debugging-port=9222]
    health: { type: tcp, port: 9222 }
```

The schema stays intentionally small. Add fields only for a real use case.

## Core reconciler

```text
config.yaml → validate → resolve (commands, paths) → desired state
  → LaunchAgent plists → running processes → health → status
```

The core is a library used by the CLI, shire-agent and the app.

### Resolving commands (important)

launchd starts services with `PATH=/usr/bin:/bin:/usr/sbin:/sbin` and does not expand `~`. On this Mac, `node` and `pnpm` come from **nvm** (`~/.nvm/versions/node/<version>/bin`), so their paths also change whenever you switch Node versions.

- On every `apply`, resolve each command to an absolute path using the login shell, and expand `~`.
- Show resolved paths in validation (`pnpm → ~/.nvm/versions/node/v22.20.0/bin/pnpm`), and warn when a command comes from a version manager.
- Each plist runs a small wrapper, `shire run <service>`, rather than the raw command. The wrapper waits for dependencies, loads `envFile` (secrets never go into the plist), runs the real command with its output captured into rotating logs, records starts and exits for crash-loop detection, and writes clear log lines (`shire: no such file: …/v22.14.0/bin/pnpm`).
- A service's PATH is its command's folder, then any PATH the config sets, then Homebrew and the system. Not the whole login PATH: that changes on every nvm switch and would restart unrelated services (Red Book) for nothing.
- `build:` runs on `apply` for new or changed services and on `shire restart`, never on launchd's automatic respawns, so a crash loop doesn't rebuild every 10 seconds. A failed build leaves the running version alone.

### Applying changes

- Compare the generated plist with the installed one; reload only services that changed. `apply` never restarts everything.
- Use `launchctl bootstrap` / `bootout`, not the deprecated `load` / `unload`.
- Show what will happen before applying ("Apply starts redbook, restarts bagend-web").
- Respect `dependsOn` for start order (postgres → api → web).

## Service kinds

- **Managed** (default): Shire generates the LaunchAgent (`com.shire.<service>`) and owns start/stop/restart.
- **External**: an existing launchd job Shire doesn't own, such as `homebrew.mxcl.postgresql@16`. Shire only watches its state and health, and can depend on it. It never edits or unloads it.
- **GUI app**: a managed service whose command is an app bundle's binary (Red Book). If the app is already open without the right flags, the relaunch just activates the existing window. Shire detects that (debug port closed while the app runs) and explains it.

## launchd

Shire owns only agents labeled `com.shire.*` in `~/Library/LaunchAgents/` and never touches others.

launchd throttles restarts (about 10 s). Shire detects repeated exits and reports **crash-looping** with the last exit code, instead of a vague "running/stopped".

## Keep-awake

A native power assertion (`IOPMAssertionCreateWithName`, prevent idle system sleep) held by shire-agent while server mode is on **and the Mac is on power**. On battery it's released so a laptop sleeps before the battery dies; `shire status` says so.

Limits, surfaced in Readiness rather than hidden: a MacBook sleeps when the lid closes unless it's on power with an external display (clamshell). Preventing that needs root, so it's out of scope for v1.

## Tailscale

Integration, not reimplementation. Shire detects install and connection state, shows hostname/IP, offers reconnect, uses `tailscale serve` for the phone page and per-service `serve:` sharing (HTTPS on the tailnet), and uses the tailnet login to authorize phone actions. It does not own VPN configuration.

## Readiness

A screen answering: **will this Mac keep serving without you?**

**Decision: FileVault stays on.** After any restart the Mac waits at the login screen until you log in, and services and Tailscale start only then. Readiness shows this as an accepted choice, not a warning. So the goal is fewer surprise restarts.

Checks:

- **macOS automatic updates** install and restart overnight. The one remaining warning: set updates to download-only.
- FileVault on (accepted: you log in after restarts)
- Restart after power failure (`pmset autorestart`)
- Idle sleep blocked (shire-agent's assertion)
- Lid / clamshell behavior
- Background items approved in Login Items
- Phone page reachable
- Mac health: disk space, AC vs battery, pending restart, uptime

Shire reports and links to the right System Settings pane. It never changes security settings itself.

## Alerts

Channels:

- **macOS notifications**
- **Phone push**: web push to the status page saved to the Home Screen. Needs HTTPS (from `tailscale serve`). Travels through Apple's push service, the one exception to strictly local.

No ntfy, no outside heartbeat. **The trade-off:** when the Mac is off or waiting at the login screen, nothing can alert you. The phone page not loading is the signal. After you log in, Shire reports how long services were down.

Rules (all on by default):

- a service crash-loops (3 restarts in 5 min)
- a service stays unhealthy (2 min, never a single failed check)
- a readiness check fails (e.g. an update is about to restart the Mac)
- services were down (reported after login, with duration)
- back to healthy (one message per recovery)

One alert per incident, then one on recovery. Open incidents are saved, so restarting the agent never repeats an alert.

Built in Phase 3 with two known gaps: per-rule on/off switches (the Alerts screen in the designs) aren't in the config yet, all rules are on and `alerts.macos` turns notifications off entirely; and notifications go through `osascript` (shown as Script Editor) until the menu bar app gives Shire a real app identity.

## Phone status page

A small web page served by shire-agent through `tailscale serve`: HTTPS, reachable only on your tailnet, no App Store app, no account. Add to Home Screen to use it like an app and receive alerts. Follows the phone's light/dark setting.

Shows: overall state, services (with **Open** links for services shared with `serve:`), keep-awake, Tailscale, readiness, likely cause, last log lines, recent events.

Actions: **Restart, on by default**, confirmed first, limited to your Tailscale login. No config changes from the phone.

## Logs

```text
~/Library/Logs/shire/<service>.stdout.log
~/Library/Logs/shire/<service>.stderr.log
```

Rotation is in the MVP: size cap (default 10 MB, keep 3). UI: last 100 lines, follow, stdout/stderr/both, reveal in Finder, clear.

## Likely cause

A fixed list of known patterns, each with a plain explanation and a fix:

| Signal | Explanation | Fix offered |
|---|---|---|
| exit 127, path missing under `~/.nvm` | nvm switched Node versions | re-resolve and restart (a normal apply) |
| exit 127 / command not found | tool not on launchd's PATH | use the resolved full path |
| `EADDRINUSE` | port already in use | show which process holds it |
| permission denied | file or folder not accessible | show the path |
| missing `cwd` | working folder doesn't exist | edit config |
| dependency down | e.g. postgres not reachable | point at the dependency |
| app running, debug port closed | Red Book opened without the flag | quit it once; Shire relaunches it with the flag |
| crash-loop right after apply | the last config change broke it | show the diff, offer revert |

No generic diagnosis engine.

---

# CLI

```bash
shire apply            # reconcile config with the system (restarts only what changed)
shire status           # services, health, keep-awake, tailscale, readiness summary
shire start|stop|restart <service>
shire logs <service> [-f]
shire validate         # schema, resolved commands, cwd, ports, dependencies
shire doctor           # readiness checks
shire run <service>    # the wrapper launchd calls (not usually run by hand)
```

```text
$ shire status
NAME              PROCESS        HEALTH
postgres          external       healthy
bagend-api        running        healthy
bagend-web        crash-looping  exit 127 (pnpm path moved: nvm)
redbook           running        port 9222 open
tailscale         running        connected
keep-awake        active         on power

readiness: 1 warning (run `shire doctor`)
```

---

# Mac app

## Menu bar

Answers one question at a glance: **is my little Mac server okay?** One icon with a green/amber/red dot. The popover shows the overall state, each service, Tailscale, keep-awake, the next action ("View bagend-web logs…"), readiness and alert status.

## Window

No overview dashboard in v1; the menu bar is the overview. The window has:

- **Service page**: state, last exit, restarts, health, "healthy until", logs, likely cause, tailnet sharing, files.
- **config.yaml**: YAML plus live checks (valid YAML, commands resolved, version-manager warning, folders, dependencies, ports) and an "Apply will…" preview. "Open in VS Code" for real editing.
- **Readiness**
- **Alerts**: channels, rules, preview, send test alert.

Light and dark follow macOS.

---

# MVP phases

## Phase 1 — Core ✅ (see README.md)

- Parse and validate YAML
- Resolve commands and paths on every apply; `shire run` wrapper
- Managed, external and GUI-app services; `dependsOn` start order
- Generate/install/remove LaunchAgents; restart only what changed
- start/stop/restart, `status`, crash-loop detection
- env, `envFile`, `build:` step, stdout/stderr logs with rotation

**Done when** `shire apply` brings up postgres (watched), the Bag End staging copy and Red Book with its debug port, and a moved nvm path shows as "crash-looping, exit 127, pnpm path moved" rather than silent failure.

## Phase 2 — Server mode and readiness ✅

shire-agent, keep-awake assertion, Tailscale status, `shire doctor`, combined overall status.

## Phase 3 — Health checks and alerts ✅

HTTP and TCP health checks in shire-agent, unhealthy/degraded states, macOS notifications with the rules above, and the "services were down" report after login.

Let launchd handle crashes. Never restart a running process because one health check failed.

## Phase 4 — Phone ✅

Status page via `tailscale serve`, Restart (on by default, confirmed, tailnet login only), web push alerts, per-service `serve:` sharing with Open links.

## Phase 5 — Menu bar app ✅

SwiftUI menu bar on the same Core library.

## Phase 6 — Window ✅

Service page, config.yaml with live checks, Readiness, Alerts. Light and dark.

---

# Decisions

| Question | Decision |
|---|---|
| Name | **Shire**: app, CLI (`shire`), agent (`shire-agent`), labels (`com.shire.*`) |
| FileVault | Stays on. Log in after restarts; Readiness treats it as accepted. |
| Phone Restart | On by default |
| Third-party services | None. Strictly local, except Apple push for phone alerts. |
| Bag End | A staging copy with sample data runs here; production runs elsewhere |
| Desktop apps | Red Book is kept open with its debug port; a copy you opened yourself is watched, not duplicated |
| Minimum macOS | **14 Sonoma**: MenuBarExtra, Observation and SMAppService are all available. (This Mac runs 26.) |

# Cut from v1

Revisit after the dogfooding week, only if missed.

- **Form-based config editor.** YAML plus live checks instead.
- **Overview dashboard window.** The menu bar and service page cover it.
- **24-hour health history.** Current state plus "healthy until" answers the question.
- **Config changes from the phone.** Phone gets Restart only.
- **Third-party alert channels** (ntfy, heartbeat), by the strictly-local decision.
- **"Likely cause" beyond the fixed pattern list.**

# Explicitly out of scope

Docker orchestration, Kubernetes, remote server management, Linux/Windows, SSH management, cron replacement, arbitrary system administration, secrets manager, general reverse proxy (per-service `tailscale serve` is the exception), DNS server, Tailscale replacement, full `launchd` editor, hosted dashboard, user accounts, changing macOS security settings, running real production data.

---

# Implementation

Swift throughout: one Core library shared by CLI, agent and app; SwiftUI for the menu bar and window. Minimum macOS 14.

```text
Sources/
├── ShireCore/
│   ├── Config/       # schema, loading, validation, command resolution
│   ├── Launchd/      # plist generation, launchctl
│   ├── Reconciler/   # plan/apply, dependency order
│   ├── Runner/       # `shire run`: the wrapper launchd starts
│   ├── Logs/         # rotation, tail
│   ├── Status/       # event log, crash loops, likely cause
│   └── Health/       # one-shot TCP/HTTP checks
└── shire/           # the CLI
# Later: Agent/ (shire-agent), App/ (menu bar + window), Readiness/, Alerts/
```

---

# First dogfooding setup

Start with the config above: postgres (external), the Bag End staging copy, and Red Book. Run it continuously for at least a week. Track what fails, what needs Terminal, what state is confusing, what information you keep wanting, and what actions you keep repeating. Those pain points drive the next features, including whether anything in "Cut from v1" comes back.

---

# Definition of success

The workflow feels boring:

1. After a restart, you log in once and everything comes up in order.
2. Surprise restarts are rare because Readiness caught the update setting.
3. Tailscale is available.
4. The Mac stays awake when server mode is on.
5. Apps restart after crashes, and a crash loop is named, explained and alerted.
6. A glance at the menu bar, or your phone, confirms everything is okay.
7. When something breaks, you hear about it without looking.
8. Configuration changes are a YAML edit plus `shire apply`.
9. You rarely need to open Terminal to babysit anything.
