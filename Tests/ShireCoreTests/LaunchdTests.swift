...
struct LaunchAgentBuilderTests {
    ...
    @Test func webServicePlist() throws {
        ...
        let web = plist(for: "bagend-web", config: config)
        #expect(web["Label"] as? String == "com.shire.bagend-web")
        #expect(web["WorkingDirectory"] as? String == "/Users/me/Code/bagend/web")
        #expect(web["KeepAlive"] as? Bool == true)
        #expect(web["RunAtLoad"] as? Bool == true)
        let args = try #require(web["ProgramArguments"] as? [String])
        #expect(Array(args.prefix(3)) == ["/Users/me/.local/bin/shire", "run", "bagend-web"])
