...
struct HealthMonitorTests {
    ...
    func config(interval: String = "30s") throws -> ShireConfig {
    ...
    @Test func forgetsServicesThatLeaveTheConfig() throws {
        ...
        monitor.runDueChecks(config: ShireConfig(), now: t0.addingTimeInterval(60))
...
struct IncidentEngineTests {
    let t0 = Date(timeIntervalSince1970: 1_800_000_000)
    let config = ShireConfig(services: ["web": ServiceConfig(command: "pnpm"), "api": ServiceConfig(command: "node")])
    ...
    @Test func downtimeReport() {
        ...
        #expect(stopped?.title == "Shire was stopped")
...
@Suite("shire-agent alerts", .serialized)
struct AgentAlertTests {
    @Test func crashLoopingServiceAlertsThroughTheAgentOnce() throws {
        ...
        let restarted = ShireAgent(paths: home.paths, power: FakePower(), powerSource: { PowerInfo(hasBattery: false, onACPower: true) },
                                    monitor: HealthMonitor(probe: { _ in .healthy(detail: "ok") }), notifier: notifier,
                                    facts: { SystemFacts(fileVaultOn: true, autoInstallMacOSUpdates: false) }, phoneServer: false)
    ...
    @Test func aReadinessBlipDoesNotAlert() throws {
        let home = try TempHome()
        try home.write(".config/shire/config.yaml", "presets: { tailscale: { enabled: true } }\n")
        let notifier = FakeNotifier()
        let flaky = LockedBox(true)
        let base = SystemFacts(fileVaultOn: true, autoInstallMacOSUpdates: false,
                               tailscale: .connected(hostName: "mbp", dnsName: nil, ipv4: nil))
        let agent = ShireAgent(paths: home.paths, power: FakePower(), powerSource: { PowerInfo(hasBattery: false, onACPower: true) },
                               monitor: HealthMonitor(probe: { _ in .healthy(detail: "ok") }), notifier: notifier,
                               facts: {
                                   var facts = base
                                   if flaky.value { facts.tailscale = .daemonNotRunning(detail: "timed out") }
                                   return facts
                               }, phoneServer: false)
        let t0 = Date()
        agent.tick(now: t0)                          // seen once: pending, no alert
        #expect(notifier.titles.isEmpty)
        flaky.value = false
        agent.tick(now: t0.addingTimeInterval(31))   // recheck 30s later: gone, never alerts
        #expect(notifier.titles.isEmpty)

        flaky.value = true
        agent.tick(now: t0.addingTimeInterval(400))  // seen again…
        agent.tick(now: t0.addingTimeInterval(431))  // …and still there: now it alerts, once
        agent.tick(now: t0.addingTimeInterval(462))
    ...
    @Test func alertsOffStillLogsButDoesntNotify() throws {
        ...
        let t0 = Date()
        agent.tick(now: t0)
        agent.tick(now: t0.addingTimeInterval(31))
        #expect(notifier.titles.isEmpty)
