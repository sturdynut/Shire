...
struct ReadinessTests {
    ...
    func config(keepAwake: Bool = true, tailscale: Bool = true) -> ShireConfig {
        ShireConfig(serverMode: ServerMode(keepAwake: keepAwake), presets: Presets(tailscale: TailscalePreset(enabled: tailscale)))
    }

    func check(_ id: String, _ facts: SystemFacts, config: ShireConfig? = nil) -> ReadinessCheck? {
    ...
    @Test func agentMissingStaleOrConfused() {
        ...
        #expect(check("agent", facts)?.title == "shire-agent isn’t installed")
        facts.agentLoaded = true
        facts.agentState = agentState(age: 120)
        #expect(check("agent", facts)?.title == "shire-agent isn’t responding")
        #expect(check("keep-awake", facts)?.level == .warn)
        facts.agentState = agentState(configError: "bad yaml")
        #expect(check("agent", facts)?.title == "shire-agent can’t read config.yaml")
    }
    ...
    @Test func systemSummary() {
        ...
        #expect(summary.rows.map(\.name) == ["shire-agent", "keep-awake", "tailscale", "readiness"])
        #expect(summary.problems.isEmpty)
        #expect(summary.rows.last?.value == "1 warning · run `shire doctor`")

        facts.agentState = nil
        facts.agentLoaded = false
        summary = SystemSummary(facts: facts, config: config(), now: now)
        #expect(summary.problems == ["shire-agent", "keep-awake"])
    }
}
...
@Suite("shire-agent", .serialized)
struct AgentTests {
    func makeAgent(_ home: TempHome, power: FakePower, source: @escaping () -> PowerInfo) -> ShireAgent {
        ShireAgent(paths: home.paths, interval: 1, power: power, powerSource: source,
    ...
    @Test func holdsKeepAwakeOnPowerAndWritesState() throws {
        ...
        try home.write(".config/shire/config.yaml", "serverMode: { keepAwake: true }\n")
        let power = FakePower()
    ...
    @Test func followsConfigChangesAndSurvivesABrokenFile() throws {
        let home = try TempHome()
        let file = try home.write(".config/shire/config.yaml", "serverMode: { keepAwake: true }\n")
        let power = FakePower()
...
struct AgentInstallTests {
    ...
    @Test func uninstallRemovesServicesAndTheAgent() throws {
        ...
        #expect(Set(removed) == ["doulasimply-api", "doulasimply-web", "tradingview", "shire-agent"])
    ...
    @Test func agentRestartsAfterShireIsUpdated() throws {
        let home = try TempHome()
        let binary = try home.write("bin/shire", "v1")
        let now = Date()
    ...
    @Test func agentNameIsReserved() throws {
        ...
        #expect(issues.contains { $0.message.contains("reserved for shire-agent") })
    }
}
