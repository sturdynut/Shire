...
struct EnvFileTests {
    @Test func parsesCommonShapes() {
        ...
        # demo database
        DATABASE_URL=postgresql://me@localhost:5432/doulasimply_demo
        ...
        =novalue
        """)
        #expect(values["DATABASE_URL"] == "postgresql://me@localhost:5432/doulasimply_demo")
...
struct StatusTests {
    ...
    @Test func likelyCausePointsAtADownDependency() throws {
        ...
        let cause = inspector.likelyCause(
            name: "doulasimply-api", service: config.services["doulasimply-api"]!,
            state: .exited(code: 75), dependencyHealth: ["postgres": .unhealthy(detail: "port 5432 closed")])
        #expect(cause == "postgres isn’t reachable (port 5432 closed), so doulasimply-api is waiting for it.")
    }
