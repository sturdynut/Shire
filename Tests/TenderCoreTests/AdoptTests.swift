...
struct AdoptTests {
    ...
    @Test func likelyCauseForAHandOff() throws {
        ...
        #expect(cause?.hasPrefix("tradingview is already open without the options Tender starts it with") == true)
    }
    ...
    @Test func plistAndValidation() throws {
        ...
        let plist = Reconciler(paths: TestPaths.home, launchControl: FakeLaunchControl())
            .desiredPlists(config: config, resolved: resolved, tenderExecutable: "/u")["tradingview"]!
...
enum TestPaths {
    static let home = TenderPaths(home: URL(fileURLWithPath: "/Users/me"))
}
