...
struct WebPushTests {
    ...
    @Test func encryptsSoTheBrowserCanDecrypt() throws {
        ...
        let payload = Data(#"{"title":"doulasimply-web is crash-looping"}"#.utf8)
        ...
        if let out = ProcessInfo.processInfo.environment["TENDER_WEBPUSH_VECTOR"] {
    ...
    @Test func vapidAuthorizationIsASignedJWT() throws {
        ...
        let header = try WebPush.authorization(endpoint: URL(string: "https://web.push.apple.com/QK4-abc")!, key: key,
                                               subject: "mailto:tender@example.com", now: Date(timeIntervalSince1970: 1_800_000_000))
...
struct PhoneServerTests {
    ...
    func setUp(actions: String = "restart") throws -> (TempHome, PhoneServer, FakeLaunchControl) {
        ...
        """
        try home.write(".config/tender/config.yaml", yaml)
        let config = try ConfigLoader.parse(yaml)
        try home.write("Library/LaunchAgents/com.tender.web.plist", "<plist/>")
        try home.write("Library/Logs/tender/web.stderr.log", "one\ntwo\nthree\n")
        let control = FakeLaunchControl(loaded: ["com.tender.web": LaunchJobInfo(state: "running", pid: 9)])
    ...
    func request(_ method: String, _ path: String, login: String? = nil, tenderHeader: Bool = false, body: Data = Data()) -> HTTPServer.Request {
        var headers: [String: String] = [:]
        if let login { headers["tailscale-user-login"] = login }
        if tenderHeader { headers["x-tender"] = "1" }
    ...
    @Test func restartOnlyForTheOwnerWithTheHeader() async throws {
        ...
        #expect(await server.handle(request("POST", "/api/services/web/restart", login: owner)).status == 403) // no X-Tender
        #expect(await server.handle(request("POST", "/api/services/web/restart", tenderHeader: true)).status == 403) // not via Tailscale
        #expect(await server.handle(request("POST", "/api/services/web/restart", login: "eve@x.com", tenderHeader: true)).status == 403)
        #expect(await server.handle(request("POST", "/api/services/db/restart", login: owner, tenderHeader: true)).status == 404) // external
        #expect(control.calls.isEmpty)

        let ok = await server.handle(request("POST", "/api/services/web/restart", login: owner, tenderHeader: true))
        #expect(ok.status == 200)
        #expect(control.calls == ["kickstart -k com.tender.web"])
    ...
    @Test func readOnlyModeRefusesRestart() async throws {
        ...
        defer { withExtendedLifetime(home) {} }
        let response = await server.handle(request("POST", "/api/services/web/restart", login: owner, tenderHeader: true))
    ...
    @Test func logsAndSubscriptions() async throws {
        ...
        #expect(await server.handle(request("POST", "/api/push/subscribe", login: "eve@x.com", tenderHeader: true, body: subscription)).status == 403)
        #expect(await server.handle(request("POST", "/api/push/subscribe", login: owner, tenderHeader: true, body: subscription)).status == 200)
        ...
        #expect(await server.handle(request("POST", "/api/push/unsubscribe", login: owner, tenderHeader: true, body: unsubscribe)).status == 200)
    ...
    @Test func httpParsing() {
        ...
        #expect(request.headers["x-tender"] == "1")
