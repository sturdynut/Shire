...
struct ReconcilerTests {
    func setUp(_ yaml: String = planConfigYAML) throws -> (TempHome, TenderConfig, FakeLaunchControl) {
    ...
    @Test func firstApplyInstallsEverythingInDependencyOrder() throws {
        ...
        #expect(control.calls == ["bootstrap com.tender.tradingview", "bootstrap com.tender.doulasimply-api", "bootstrap com.tender.doulasimply-web"])
        #expect(builder.log.all == ["doulasimply-api: pnpm build", "doulasimply-web: pnpm build"])
        #expect(FileManager.default.fileExists(atPath: home.paths.plist(forLabel: "com.tender.tradingview").path))
    }
    ...
    @Test func nvmSwitchRestartsOnlyTheServicesThatUseIt() throws {
        let (home, config, control) = try setUp()
        let reconciler = Reconciler(paths: home.paths, launchControl: control, builder: FakeBuilder(succeeds: true))
        let first = reconciler.desiredPlists(config: config, resolved: planResolution(), tenderExecutable: "/u")
        ...
        let second = reconciler.desiredPlists(config: config, resolved: moved, tenderExecutable: "/u")
    ...
    @Test func failedBuildLeavesTheRunningVersionAlone() throws {
        ...
        #expect(!control.calls.contains("bootstrap com.tender.doulasimply-api"))
        #expect(control.calls.contains("bootstrap com.tender.tradingview"))
    }
    ...
    @Test func servicesRemovedFromConfigAreStoppedAndRemoved() throws {
        ...
        let plan = reconciler.plan(config: smaller, desired: reconciler.desiredPlists(config: smaller, resolved: planResolution(), tenderExecutable: "/u"))
        ...
        #expect(control.calls.last == "bootout com.tender.tradingview")
        #expect(!FileManager.default.fileExists(atPath: home.paths.plist(forLabel: "com.tender.tradingview").path))
    }
    ...
    @Test func stoppedServiceIsStartedAgainWithoutRebuilding() throws {
        ...
        let desired = reconciler.desiredPlists(config: config, resolved: planResolution(), tenderExecutable: "/u")
        _ = reconciler.apply(reconciler.plan(config: config, desired: desired), config: config, resolved: planResolution(), build: true)
        try control.bootout("com.tender.doulasimply-web")
        ...
        #expect(control.isLoaded("com.tender.doulasimply-web"))
    }
}
