...
enum PhonePage {
    ...
    self.addEventListener('push', (event) => {
      let data = {};
      try { data = event.data ? event.data.json() : {}; } catch (e) { data = { title: 'Tender', body: event.data ? event.data.text() : '' }; }
      const title = data.title || 'Tender';
      event.waitUntil(self.registration.showNotification(title, {
        body: data.body || '',
        tag: (data.service || 'tender') + ':' + (data.kind || ''),
    ...
    <meta name="apple-mobile-web-app-title" content="Tender">
    ...
    * { box-sizing: border-box; }
    ...
    function render() {
      ...
      document.title = route ? decodeURIComponent(route[1]) + ' · Tender' : 'Tender · ' + status.host;
    }

    function header() {
      return `<header><div class="who"><div class="eyebrow">Tender · via Tailscale</div><h1>${esc(status.host)}</h1></div>
    ...
    function serviceView(name) {
      ...
      const fragment = $(`<a class="back" href="#/">‹ ${esc(status.host)}</a>
        ...
        ${x.external ? '<p class="note">Tender only watches this one; its own tools manage it.</p>' : `<h2>stderr · last lines</h2><pre class="logs mono" id="logs">Loading…</pre>`}
    ...
    function confirmRestart(name) {
      ...
      document.getElementById('sheet-yes').onclick = async () => {
        close();
        const response = await fetch(`/api/services/${encodeURIComponent(name)}/restart`, { method: 'POST', headers: { 'X-Tender': '1' } });
    ...
    function pushSection() {
      ...
      if (Notification.permission === 'granted' && localStorage.getItem('tender-subscribed') === 'yes') {
        return '<div>Alerts are on for this phone.</div><button class="secondary" onclick="testPush()">Send a test alert</button>';
      }
      if (Notification.permission === 'denied') return '<div>Notifications are blocked for this app in iOS Settings → Notifications → Tender.</div>';
    ...
    async function enablePush() {
      try {
        ...
        const response = await fetch('/api/push/subscribe', { method: 'POST', headers: { 'X-Tender': '1', 'Content-Type': 'application/json' }, body: JSON.stringify(subscription) });
        if (!response.ok) throw new Error((await response.json().catch(() => ({}))).message || 'The Mac refused the subscription.');
        localStorage.setItem('tender-subscribed', 'yes');
    ...
    async function testPush() {
      const response = await fetch('/api/push/test', { method: 'POST', headers: { 'X-Tender': '1' } });
