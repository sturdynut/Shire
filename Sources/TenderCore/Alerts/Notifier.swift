...
public struct OsascriptNotifier: Notifying {
    ...
    public func deliver(_ alert: AlertMessage) {
        let script = "display notification \(Self.quote(alert.body)) with title \(Self.quote("Tender")) subtitle \(Self.quote(alert.title))"
...
/// Every alert, delivered or not, as JSON lines. `tender alerts` reads it; the phone page will too.
