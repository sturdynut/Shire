    ...
    var body: some Scene {
        MenuBarExtra {
            ...
            Image(nsImage: StatusIcon.image(for: delegate.model.overall))
                .accessibilityLabel("Tender: \(delegate.model.headline)")
        }
...
/// Starts Tender at login, so the menu bar is there whenever the services are.
enum LoginItem {
    ...
    static func set(_ enabled: Bool) {
        do {
            ...
            NSLog("Tender: couldn’t change the login item: \(error)")
        }
    }
...
enum StatusIcon {
    static func image(for overall: TenderSnapshot.Overall) -> NSImage {
        let dot: NSColor
        switch overall {
...
final class MainWindowController {
    private let model: TenderModel
    private var window: NSWindow?

    init(model: TenderModel) {
        self.model = model
    }
    ...
    func show() {
        if window == nil {
            ...
            window.center()
            window.setFrameAutosaveName("TenderMain")
            self.window = window
        }
