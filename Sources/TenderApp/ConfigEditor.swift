...
/// config.yaml, edited in place with live checks. Tender watches the file, so edits in another editor show up here too.
struct ConfigEditor: View {
    ...
    private func checkCommandsAndPlan() {
        ...
        Task.detached {
            ...
            let dryRun = TenderCLI.run(["apply", "--dry-run"], title: "Plan")
