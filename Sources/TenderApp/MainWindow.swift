...
struct ServiceDetail: View {
        ...
        if let service = model.service(name) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if service.config.isExternal {
                    Text("Tender only watches \(service.config.external ?? name); its logs live wherever that job writes them.")
    ...
    private func header(_ service: TenderSnapshot.Service) -> some View {
    ...
    private func commandLine(_ service: TenderSnapshot.Service) -> String {
    ...
    private func stats(_ service: TenderSnapshot.Service) -> some View {
    ...
    private func processDetail(_ service: TenderSnapshot.Service) -> String {
    ...
    private func healthCheckText(_ service: TenderSnapshot.Service) -> String {
    ...
    private func logs(_ service: TenderSnapshot.Service) -> some View {
