...
public enum Readiness {
    ...
    public static func evaluate(_ facts: SystemFacts, config: ShireConfig, now: Date = Date()) -> [ReadinessCheck] {
    ...
    static func agent(_ facts: SystemFacts, now: Date) -> ReadinessCheck {
        guard facts.agentLoaded else {
            return ReadinessCheck("agent", .warn, "shire-agent isn’t installed", "Keep-awake, health checks and alerts need it.",
                                  fix: "Run `shire apply`.")
        }
        guard let state = facts.agentState, state.isFresh(now: now) else {
            return ReadinessCheck("agent", .warn, "shire-agent isn’t responding", "It’s loaded but hasn’t reported in the last \(Int(AgentState.staleAfter)) seconds.",
                                  fix: "Check `~/Library/Logs/shire/shire-agent.log`, then `shire apply`.")
        }
        if let error = state.configError {
            return ReadinessCheck("agent", .warn, "shire-agent can’t read config.yaml", "\(error) It keeps the last good settings.", fix: "Run `shire validate`.")
        }
        return ReadinessCheck("agent", .ok, "shire-agent is running", "pid \(state.pid), approved as a background item.")
    }

    static func keepAwake(_ facts: SystemFacts, config: ShireConfig, now: Date) -> ReadinessCheck {
        ...
        guard let state = facts.agentState, state.isFresh(now: now) else {
            return ReadinessCheck("keep-awake", .warn, "Nothing is keeping the Mac awake", "shire-agent holds the keep-awake, and it isn’t running.", fix: "Run `shire apply`.")
        }
        if state.keepAwake.held {
            return ReadinessCheck("keep-awake", .ok, "Idle sleep is blocked", "shire-agent holds a power assertion (\(state.keepAwake.reason)). The display can still sleep.")
        }
    ...
    static func disk(_ facts: SystemFacts) -> ReadinessCheck? {
        ...
        if free < lowDiskBytes {
            return ReadinessCheck("disk", .warn, "Low on disk space: \(text) free", "Logs, builds and databases fail in confusing ways when the disk fills.",
                                  fix: "Free some space; `shire logs` files rotate, but other apps’ may not.")
        }
