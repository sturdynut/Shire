...
/// The SYSTEM rows under `shire status`: shire-agent, keep-awake, Tailscale and the readiness count.
public struct SystemSummary: Sendable {
    ...
    public init(facts: SystemFacts, config: ShireConfig, now: Date = Date()) {
        let fresh = facts.agentState.flatMap { $0.isFresh(now: now) ? $0 : nil }
        if let state = fresh {
            add("shire-agent", "running · pid \(state.pid)", .ok)
        } else if facts.agentLoaded {
            add("shire-agent", "not responding", .warn, problem: true)
        } else {
            add("shire-agent", "not installed · run `shire apply`", .warn, problem: true)
        }
        ...
        if !config.serverMode.keepAwake {
            ...
            add("keep-awake", "inactive · shire-agent isn’t running", .warn, problem: true)
        }
        ...
        if warnings == 0 {
            ...
            add("readiness", "\(warnings) warning\(warnings == 1 ? "" : "s") · run `shire doctor`", .warn)
        }
    }
