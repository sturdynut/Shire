...
/// Something Shire has already told you about. One alert opens it, at most one recovery message closes it.
...
/// What shire-agent saw for one service on this tick.
...
public enum IncidentEngine {
    public static func evaluate(previous: IncidentState, services: [String: ServiceObservation], readinessWarnings: [ReadinessCheck]?,
                                config: ShireConfig, now: Date) -> (IncidentState, [AlertMessage]) {
        var state = previous
    ...
    /// The "services were down" report, made when shire-agent starts and finds a gap since it last reported.
    public static func downtime(previousHeartbeat: Date?, bootTime: Date?, agentStart: Date, minimumGap: TimeInterval = 120) -> AlertMessage? {
        ...
        return AlertMessage(time: agentStart, kind: .info, title: "Shire was stopped",
                            body: "shire-agent wasn’t running for about \(Readiness.describe(gap)), since \(since). Services it watches weren’t being checked.")
    }
