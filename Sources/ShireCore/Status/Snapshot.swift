...
/// Everything `shire status`, the menu bar and the phone page show, computed one way in one place.
public struct ShireSnapshot: Sendable {
    public enum Overall: Equatable, Sendable {
        case healthy
        case attention(count: Int)
        /// No config, or shire-agent isn't running, so nothing is being looked after.
    ...
    public struct Service: Identifiable, Sendable {
        ...
        /// "for 3m" when failing, "checked 12s ago" when healthy (only when shire-agent tracks it).
    ...
    public var overall: Overall {
        ...
        if agent == nil { return .notRunning(reason: "shire-agent isn’t running") }
    ...
    public static func build(paths: ShirePaths, launchControl: LaunchControl = SystemLaunchControl(),
                             includeSystem: Bool = true, facts: SystemFacts? = nil, now: Date = Date()) async -> ShireSnapshot {
        ...
        let config: ShireConfig
        do {
            config = try ConfigLoader.load(from: paths.configFile)
        } catch {
            return ShireSnapshot(time: now, configError: String(describing: error), config: nil, services: [], agent: agent,
        ...
        // Cached facts can predate the agent's latest heartbeat; its own state is cheap to read, so never use a stale copy.
        if gathered != nil {
            gathered!.agentState = AgentState.read(from: paths.agentStateFile)
            gathered!.agentLoaded = launchControl.isLoaded(LaunchAgentBuilder.agentLabel)
        }
        ...
        return ShireSnapshot(time: now, configError: nil, config: config, services: services, agent: agent, facts: gathered,
