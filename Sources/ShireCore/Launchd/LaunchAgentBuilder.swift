...
public struct AgentInputs {
    ...
    public var shireExecutable: String
    ...
    public init(name: String, service: ServiceConfig, resolvedCommand: String, servicePath: String,
                shireExecutable: String, paths: ShirePaths, logs: LogSettings, dependencyWaits: [HostPort]) {
        self.name = name
        self.service = service
        self.resolvedCommand = resolvedCommand
        self.servicePath = servicePath
        self.shireExecutable = shireExecutable
        self.paths = paths
        self.logs = logs
...
public enum LaunchAgentBuilder {
    public static let labelPrefix = "com.shire."

    public static func label(for service: String) -> String { labelPrefix + service }

    /// shire-agent's label. The service name `agent` is reserved so the two can't collide.
    public static let agentLabel = labelPrefix + reservedServiceName
    public static let reservedServiceName = "agent"

    /// The service a Shire label belongs to; nil for other labels and for shire-agent itself.
    ...
    /// shire-agent's own LaunchAgent: always running, restarted if it ever exits.
    public static func agentPlist(executable: String, paths: ShirePaths) -> [String: Any] {
        [
            "Label": agentLabel,
    ...
    /// The plist launchd runs. It always starts `shire run`, which owns logging, env files and dependency waits,
    ...
    public static func plist(_ inputs: AgentInputs) -> [String: Any] {
        ...
        var runArguments = [
            inputs.shireExecutable, "run", inputs.name,
        ...
        environment["SHIRE_SERVICE"] = inputs.name
    ...
    private static func programArgumentReasons(_ before: [String], _ after: [String]) -> [String] {
        ...
        if a.head.first != b.head.first { reasons.append("shire moved to \(b.head.first ?? "?")") }
