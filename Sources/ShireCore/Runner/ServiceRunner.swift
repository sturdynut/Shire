...
/// `shire run`: what launchd actually starts. It waits for dependencies, loads the env file, runs the real
...
public final class ServiceRunner: @unchecked Sendable {
    ...
    private func runChild() -> Int32 {
        ...
        if !fm.fileExists(atPath: options.command) {
            ...
            if let manager = CommandResolver.versionManager(forPath: options.command) {
                message += " (\(manager) switched versions since the last apply; run `shire apply` to re-resolve)"
            }
        ...
        default:
            ...
            if code == 0, let adopt = options.adopt, runtime.seconds < 5, !stopRequestedSnapshot(), !HealthProbe.tcpConnect(adopt, timeout: 1) {
                say("\(options.name) exited right away without opening \(adopt). It’s probably already open without the options Shire starts it with; quit it once and Shire will start it properly.", error: true)
    ...
    private func say(_ message: String, error: Bool = false) {
        let line = "\(Self.timestamp.string(from: Date())) shire: \(message)"
