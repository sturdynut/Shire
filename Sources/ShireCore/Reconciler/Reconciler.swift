...
public struct PlannedChange {
    ...
    /// The name shire-agent appears under in plans and output.
    public static let agentName = "shire-agent"
...
public struct Reconciler: Sendable {
    ...
    public init(paths: ShirePaths, launchControl: LaunchControl, builder: ServiceBuilding = ShellServiceBuilder()) {
        self.paths = paths
    ...
    public func desiredPlists(config: ShireConfig, resolved: ResolvedEnvironment, shireExecutable: String) -> [String: [String: Any]] {
        ...
        for name in config.managedServiceNames {
            ...
            let inputs = AgentInputs(
                ...
                shireExecutable: shireExecutable,
                paths: paths,
                logs: config.logs,
    ...
    /// Names of services whose LaunchAgent Shire installed earlier.
    ...
    /// `agentPlist` is shire-agent's LaunchAgent; nil leaves the agent out of the plan (tests of services alone).
    public func plan(config: ShireConfig, desired: [String: [String: Any]], agentPlist: [String: Any]? = nil) -> ApplyPlan {
        ...
        if let agentPlist {
            ...
            if agent.action == .unchanged,
               Self.agentIsOutdated(agentStartedAt: AgentState.read(from: paths.agentStateFile)?.startedAt, executable: executable) {
                agent.action = .update(reasons: ["shire was updated"])
            }
    ...
    /// True when the shire binary was replaced after the running agent started, so the agent runs old code.
    ...
    public func apply(_ plan: ApplyPlan, config: ShireConfig, resolved: ResolvedEnvironment, build: Bool,
    ...
    private func install(_ change: PlannedChange, config: ShireConfig, resolved: ResolvedEnvironment, build: Bool,
    ...
    /// Stops and removes every LaunchAgent Shire installed, shire-agent included. Logs and config stay.
