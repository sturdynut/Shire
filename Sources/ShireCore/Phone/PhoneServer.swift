...
/// The phone page's server, run inside shire-agent. It listens on 127.0.0.1 only; `tailscale serve` puts it on your
...
public final class PhoneServer: @unchecked Sendable {
    public typealias ConfigProvider = @Sendable () -> ShireConfig?
    public typealias FactsProvider = @Sendable () -> SystemFacts?

    private let paths: ShirePaths
    ...
    public init(paths: ShirePaths, owner: String?, launchControl: LaunchControl = SystemLaunchControl(),
    ...
    func handle(_ request: HTTPServer.Request) async -> HTTPServer.Response {
        ...
        if post, parts == ["api", "push", "test"] {
            guard isOwner(request), request.headers["x-shire"] == "1" else { return forbidden("Only this Mac’s owner can do that.") }
            await onPushTest(AlertMessage(time: Date(), kind: .info, title: "Test alert", body: "Shire’s alerts reach this phone."))
            return .json(["ok": true])
        }
    ...
    func status(viewer login: String?) async -> PhoneStatus {
        let snapshot = await ShireSnapshot.build(paths: paths, launchControl: launchControl, includeSystem: false, facts: facts())
        ...
        case .notRunning: (headline, tone) = ("Shire isn’t running", "off")
        }
    ...
    private func restart(_ name: String, request: HTTPServer.Request) -> HTTPServer.Response {
        guard request.headers["x-shire"] == "1" else { return forbidden("Missing X-Shire header.") }
    ...
    private func subscribe(_ request: HTTPServer.Request) -> HTTPServer.Response {
        guard request.headers["x-shire"] == "1", isOwner(request) else { return forbidden("Only this Mac’s owner can turn on alerts.") }
    ...
    private func unsubscribe(_ request: HTTPServer.Request) -> HTTPServer.Response {
        guard request.headers["x-shire"] == "1", isOwner(request) else { return forbidden("Only this Mac’s owner can change alerts.") }
