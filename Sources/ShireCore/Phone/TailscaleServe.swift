...
/// Shares the phone page on your tailnet with `tailscale serve`, touching only Shire's own HTTPS port.
public struct TailscaleServe: Sendable {
    ...
    /// Stops sharing Shire's port, only if it still points at Shire.
