...
public struct Validator: Sendable {
    ...
    public func validate(_ config: ShireConfig, resolved: ResolvedEnvironment? = nil) -> [ValidationIssue] {
        ...
        for name in names {
            ...
            if name == LaunchAgentBuilder.reservedServiceName {
                issues.append(.init(.error, name, "“\(name)” is reserved for shire-agent; pick another service name."))
            }
            ...
            for dependency in service.dependsOn {
                if dependency == name {
                    ...
                    issues.append(.init(.warning, name, "depends on “\(dependency)”, which has no health check, so Shire can’t wait for it to be ready."))
                }
            }
            if let command = service.command, let resolution = resolved?.commands[command] {
                if resolution.path == nil {
                    ...
                    issues.append(.init(.warning, name, "\(what). The path changes when you switch versions; Shire re-resolves it on every apply."))
                }
            }
        }
    ...
    private func checkShape(_ name: String, _ service: ServiceConfig) -> [ValidationIssue] {
        ...
        if service.isExternal {
            ...
            for (key, present) in managedOnly where present {
                issues.append(.init(.error, name, "“\(key)” doesn’t apply to an external service; Shire only watches “\(service.external!)”."))
            }
        } else if service.command == nil || service.command!.isEmpty {
            issues.append(.init(.error, name, "needs a “command” (or “external:” to watch a launchd job Shire doesn’t own)."))
        }
        if service.adoptRunning, service.health?.endpoint == nil {
            issues.append(.init(.error, name, "“adoptRunning” needs a health check with a port, so Shire can tell a copy is already running."))
        }
    ...
    private func checkPortClashes(_ config: ShireConfig) -> [ValidationIssue] {
        var issues: [ValidationIssue] = []
