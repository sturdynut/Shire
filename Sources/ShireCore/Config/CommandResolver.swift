...
public struct CommandResolver: Sendable {
    ...
    func askLoginShell(for commands: [String]) throws -> ShellAnswer {
        ...
        for command in commands where Self.isSafeCommandName(command) {
            script += "printf '__SHIRE__%s=%s\\n' '\(command)' \"$(\(lookup) '\(command)' 2>/dev/null)\"; "
        }
        script += "printf '__SHIRE_PATH__=%s\\n' \"$PATH\""
    ...
    static func parse(_ output: String) -> ShellAnswer {
        ...
        for line in output.split(separator: "\n", omittingEmptySubsequences: true) {
            if line.hasPrefix("__SHIRE_PATH__=") {
                path = String(line.dropFirst("__SHIRE_PATH__=".count))
            } else if line.hasPrefix("__SHIRE__"), let eq = line.firstIndex(of: "=") {
                let name = String(line[line.index(line.startIndex, offsetBy: "__SHIRE__".count)..<eq])
