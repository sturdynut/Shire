...
/// What shire-agent last did, written every tick so `shire status` and `shire doctor` can read it.
...
/// shire-agent: the always-running part of Shire. It keeps the Mac awake, checks each service's health on its own
...
public final class ShireAgent {
    public let paths: ShirePaths
    ...
    private var config: ShireConfig?
    ...
    private let shared = LockedBox<(config: ShireConfig?, facts: SystemFacts?)>((nil, nil))
    ...
    public init(paths: ShirePaths, interval: TimeInterval = 5, power: PowerAsserting = PowerAssertion(),
    ...
    private func evaluateIncidents(config: ShireConfig, health: [String: ServiceHealth], now: Date) {
        ...
        if lastReadiness == nil || now.timeIntervalSince(lastReadiness!) >= every {
            ...
            let seen = Readiness.warnings(Readiness.evaluate(gathered, config: config, now: now))
    ...
    private func send(_ alert: AlertMessage) {
        ...
        // The menu bar app reads alerts.jsonl and posts them as Shire; only fall back when it isn't running.
    ...
    private func updatePhoneServer(config: ShireConfig) {
    ...
    public func runForever() -> Never {
        ...
        log("shire-agent started (config: \(paths.configFile.path))")
        reportDowntime()
        ...
        power.release()
        log("shire-agent stopping; keep-awake released")
        exit(0)
    }
    ...
    private func updateKeepAwake() -> AgentState.KeepAwake {
        ...
        if power.hold(reason: "Shire server mode: keeping this Mac awake for its services") {
