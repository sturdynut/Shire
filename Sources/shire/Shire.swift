...
@main
struct Shire: AsyncParsableCommand {
...
struct Apply: ParsableCommand {
    ...
    func run() throws {
        ...
        if executable.contains("/.build/") {
            print(Terminal.amber("note") + "  LaunchAgents will point at \(executable), a build folder. Install shire (make install) before relying on it.")
        }

        let reconciler = Reconciler(paths: paths, launchControl: SystemLaunchControl())
        let desired = reconciler.desiredPlists(config: config, resolved: resolved, shireExecutable: executable)
    ...
    /// `remote.statusPage: tailnet` shares shire-agent's page with `tailscale serve`; `off` stops sharing it.
    private func sharePhonePage(config: ShireConfig, width: Int) {
...
struct Status: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        let snapshot = await ShireSnapshot.build(paths: options.paths)
...
private func managedService(_ name: String, in config: ShireConfig) throws -> ServiceConfig {
    guard let service = config.services[name] else {
        let known = config.services.keys.sorted().joined(separator: ", ")
        throw ShireError("No service named “\(name)”. Known services: \(known).")
    }
    if let external = service.external {
        throw ShireError("\(name) is external (\(external)); Shire only watches it. Use the tool that owns it, e.g. brew services.")
    }
    return service
}
...
struct Start: ParsableCommand {
    ...
    func run() throws {
        ...
        guard FileManager.default.fileExists(atPath: url.path) else {
            throw ShireError("\(target.service) hasn’t been applied yet. Run `shire apply`.")
        }
...
struct Stop: ParsableCommand {
    ...
    func run() throws {
        ...
        print("\(Terminal.green("✓")) \(target.service) stopped. It starts again on `shire start \(target.service)` or `shire apply`.")
    }
}
    ...
    func run() throws {
        ...
        if !noBuild, let build = service.build {
            ...
            if !ShellServiceBuilder().build(name: target.service, command: build, cwd: cwd,
                ...
                throw ShireError("build failed; \(target.service) was left running as it was.")
            }
        }
        if control.isLoaded(label) {
...
struct Logs: ParsableCommand {
    ...
    func run() throws {
        ...
        guard config.services[target.service] != nil else {
            throw ShireError("No service named “\(target.service)”.")
        }
        if let external = config.services[target.service]?.external {
            throw ShireError("\(target.service) is external (\(external)); its logs are wherever its own LaunchAgent writes them.")
        }
        var files: [URL] = []
...
struct Run: ParsableCommand {
    static let configuration = CommandConfiguration(
        abstract: "Run a service in the foreground with Shire's logging (this is what LaunchAgents call).",
        shouldDisplay: false
    )
...
struct Doctor: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Check whether this Mac will keep serving without you: updates, login, power, lid, Tailscale, shire-agent.")
    @OptionGroup var options: ConfigOption
...
struct Agent: ParsableCommand {
    static let configuration = CommandConfiguration(
        abstract: "Run shire-agent in the foreground (its LaunchAgent calls this).",
        shouldDisplay: false
    )
    @OptionGroup var options: ConfigOption
    ...
    func run() throws {
        ShireAgent(paths: options.paths, interval: interval).runForever()
    }
}
...
struct Uninstall: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Stop and remove every LaunchAgent Shire installed, shire-agent included. Config and logs are kept.")
    @OptionGroup var options: ConfigOption
    ...
    func run() throws {
        ...
        if outcomes.isEmpty {
            print("Nothing to remove: no Shire LaunchAgents are installed.")
            return
        }
        var failed = false
        for outcome in outcomes {
...
struct Alerts: ParsableCommand {
    ...
    @Flag(help: "Send a test notification to check macOS shows Shire's alerts.")
    var test = false
